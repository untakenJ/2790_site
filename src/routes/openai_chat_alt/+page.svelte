<script lang="ts">
  ...
  import { deserialize } from '$app/forms';
    ...
    function extractTextBetweenTags(input: string, tagName: string): string[] {
    ...
    let chatMessagesBackend: { role: string; content: string }[] = [
        { 
            ...
            1, The detailed description of the uploaded image. Put it between labels <desciption> and </desciption>.
            ...
            4, The recommented prompt for Dalle-3 based on the sketch and user feedback. Put it between labels <prompt> and </prompt>.
            `,
        },
    ]; // the message list maintained by the system is not always the same as the displayed chat message
...
<style>
    ...
    .chat-messages {
        max-height: 400px;
        overflow-y: auto;
        padding-bottom: 15px; /*originally 15*/
    }

    .chat-message {
        padding-bottom: 10px; /*originally 10*/
        display: flex;
    ...
    .chat-message p{
        padding: 10px 15px; /*originally 10 15*/
        border-radius: 20px;
        max-width: 80%;
    ...
    */
    ...
    .input-group, .feedback-section, .chat-input, .image-upload-input {
        display: flex;
        flex-direction: column;
        gap: 15px;
    }
            ...
            {#each chatMessagesChat as message}
                <div class="chat-message {message.role == "user" ? 'user' : 'bot'}">
                    {#if message.role == "assistant" && message.content != "Typing..."}
                        ...
                        {#each extractTextBetweenTags(input=message.content, tagName="description") as d}
                            <p>{d}</p>
                        {/each}
                        <h4>Feeling</h4>
                        <h4>Suggestions</h4>
