...
<script lang="ts">
    ...
    async function prepareImageMessage(userInput: string): Promise<{optimizedPrompt: string, endpoint: string}> {
        ...
        if (userInput === '' || currentImageUrl === ''){return {optimizedPrompt: userInput, endpoint: "/generate/sd3"};}
            ...
            const routingQuery = fetch('https://api.openai.com/v1/chat/completions', {
                ...
                body: JSON.stringify({
                    ...
                    { 
                        ...
                        /edit/inpaint : modifies images by filling or replacing specified areas with new content
                        /edit/outpaint : inserts additional content into an image to fill space in any direction
    ...
    async function callStability() {
        ...
        error = '';
        api_response = null;
        console.log("Initial Prompt: ", input_value);
    ...
    // Helper function to map chat messages to OpenAI's format
    function mapChatMessagesToOpenAI(messages: { user: boolean; text: string }[]): { role: string; content: string }[] {
        return messages
            .filter(message => message.text !== "Typing..." && message.text !== "Analyzing image...") // Exclude temporary messages
            .map(message => ({
    ...
    async function sendChatMessage() {
        ...
        console.log("current Input: ", chatInput)
        console.log('chatMessagesChat before mapping:', chatMessagesChat);
        ...
        */
        ...
        try {
            ...
            const userMessage = [
                { type: "text", text: "Please use the contents of this image to answer the most recent user query." },
                {
                    type: "image_url",
                    image_url: {
                        url: currentImageUrl
